...
Read [the BCQuality design guidance](.agents/skills/aldc/references/templates/bcquality-design-guidance.md)
...
The `> **Skills applied**:` line at the top of the architecture document is **mandatory**. Format and placement are defined in `.agents/skills/aldc/references/templates/architecture-template.md`. List only skills actually loaded; write "None (general architecture patterns only)" if no skill was applied. The Conductor and Review Subagent use this line to verify skill coverage downstream. Next to it, the `> **BCQuality**:` evidence line defined in the design guidance is mandatory whenever the corpus was read, and states `loaded`, never `executed`. Write `> **BCQuality**: not consulted (<reason>)` otherwise.
...
When you write `{req_name}.architecture.md`, **read and fill `.agents/skills/aldc/references/templates/architecture-template.md`**. The template is immutable and is the single source of truth for: required sections (14 for MEDIUM/HIGH), document header (Date, Complexity, Author, Status), the `> Skills applied:` traceability line, and the Status lifecycle (`Proposed` → `Approved` → `Implemented` → `Superseded`).
