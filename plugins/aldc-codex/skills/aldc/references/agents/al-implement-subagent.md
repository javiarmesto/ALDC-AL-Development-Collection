...
- Read the full `.agents/plans/{req_name}/{req_name}.spec.md`, `.architecture.md`, or `.test-plan.md` **only if** a detail referenced in the excerpt is missing (the Conductor includes the paths for this) — do not re-read them wholesale by default
...
These skills live in `.agents/skills/aldc/references/skills/`. They are **not** auto-loaded in subagent runtime — **you load them on demand** (read the `SKILL.md`) when the phase enters the matching domain. The Conductor hints the likely ones; load the one you actually need (and any other you discover you need):
