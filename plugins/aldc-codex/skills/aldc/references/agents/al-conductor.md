...
Provide this status in your responses to keep the user informed. Use the the available planning tool (or the plan document) tool to track progress.
...
**You delegate to** (via subagent delegation tool):
...
- **Architecture needed** → Use the subagent delegation tool to delegate to agent `al-architect` with context: "Design solution architecture for this requirement"
- **Quick adjustments** → Use the subagent delegation tool to delegate to agent `al-developer` with context: "Make quick adjustments to the implementation"
