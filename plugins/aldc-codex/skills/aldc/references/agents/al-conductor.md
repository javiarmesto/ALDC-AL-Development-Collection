...
> The three style guides above are the **single source of truth** at runtime. The files under `.agents/skills/aldc/references/templates/` (plan-template.md, phase-complete-template.md, plan-complete-template.md) are kept as a human reference but the conductor must NOT read them during orchestration — the format is already inline here.
...
Include **"Skills Utilization Summary"** aggregating all phases (see `.agents/skills/aldc/references/templates/plan-complete-template.md`).
...
> Scope: this governs the per-phase implement/review invocations. The same principle now covers the **BCQuality task-context** — you build it (per `.agents/skills/aldc/references/templates/bcquality-task-context.md`) and pass it inline, since you already hold `app.json` and the phase's changed objects. The review subagent still reads the selected BCQuality provider itself (the knowledge files), but no longer re-derives the task-context.
