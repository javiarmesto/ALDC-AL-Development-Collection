...
### Your runtime tools (Codex harness)

You run in the **Codex harness**, not VS Code. Use these — the VS Code AL extension commands and Copilot `#…` context-variables are not available here.

#### Build & compile (the AL CLI — ALTool)
- **`shell: al compile`**: Compile/package the current AL project into a `.app`; read the compiler output for errors. Check the installed version/help before using additional capabilities.
- **`shell: al workspace compile`**: Compile a multi-project workspace in dependency order.
...
[Load skill-testing from .agents/skills/aldc/references/skills/]
```
...
```
# Build & Validate
shell: al compile                       # Compile + package; read output for errors
shell: al workspace compile             # Multi-project workspace
...
shell: git diff                         # See what changed
...
- **Architecture design needed** → Use the subagent delegation tool to delegate to agent `al-architect` with context: "Design solution architecture for this requirement"
- **TDD orchestration needed** → Use the subagent delegation tool to delegate to agent `al-conductor` with context: "Orchestrate TDD implementation for this feature"
