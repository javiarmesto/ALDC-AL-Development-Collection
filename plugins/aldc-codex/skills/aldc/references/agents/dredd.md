...
Load [the shared review pipeline](.agents/skills/aldc/references/skills/skill-al-review-pipeline/GUIDE.md).
...
Aggregate everything into one **Audit-Report JSON** (a DO findings-report + an `audit` envelope). Shares finding semantics with [the review report contract](.agents/skills/aldc/references/templates/review-report-contract.md); no historical planning documents are required.
