...
3. **Delegate Research**: Use the `agent` tool to invoke the **agent `al-planning-subagent`** for comprehensive context gathering.
...
1. Use the `agent` tool to invoke the **al-implement-subagent** with:
...
1. Use the `agent` tool to invoke the **agent `al-review-subagent`** with:
...
Provide this status in your responses to keep the user informed. Use the the available task-list tool (or update the plan document) tool to track progress.
...
**You delegate to** (via agent tool):
...
Instead, **pass phase-relevant excerpts inline** in the `agent` instruction:
...
- **Architecture needed** → Use the agent tool to delegate to agent `al-architect` with context: "Design solution architecture for this requirement"
- **Quick adjustments** → Use the agent tool to delegate to agent `al-developer` with context: "Make quick adjustments to the implementation"
