...
description: |
  Internal quality assurance subagent for Business Central AL code. Only invoked by al-conductor via agent tool. Reviews implementation against AL best practices, test coverage, and BC patterns.
tools:
  - read
  - search
  - execute
...
**Use:**
- `execute: git diff` / `git status` - See what was modified/created
...
**CAN:**
- Analyze code changes and diffs (`execute: git diff`)
