...
```markdown
**Action**: Use `task` tool to invoke AL Architecture & Design Specialist
...
- `{req_name}.architecture.md` → Created by al-architect (COPY from `${PLUGIN_ROOT}/docs/templates/architecture-template.md`)
- `{req_name}.spec.md` → Created by al-spec.create (COPY from `${PLUGIN_ROOT}/docs/templates/spec-template.md`)
...
This agent draws on the following skill from ${PLUGIN_ROOT}/skills/. It is **not** auto-loaded — **load the `SKILL.md` on demand** (read it) when estimating:
