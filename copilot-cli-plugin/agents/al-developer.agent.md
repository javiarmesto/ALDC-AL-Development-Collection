...
- ✅ Compile/package the extension with `execute: al compile` (ALTool) and read the compiler output
- ✅ Search the codebase (`search`/`search`) and query AL symbols, definitions, and references via **al-symbols-mcp**
- ✅ Run terminal commands (`execute`) for AL build and git operations
...
### Your runtime tools (Copilot CLI harness)

You run in the **Copilot CLI harness**, not VS Code. Use these — the VS Code AL extension commands and Copilot `#…` context-variables are not available here.

#### Build & compile (the AL CLI — ALTool)
- **`execute: al compile`**: Compile/package the current AL project into a `.app`; read the compiler output for errors. Check the installed version/help before using additional capabilities.
- **`execute: al workspace compile`**: Compile a multi-project workspace in dependency order.
...
- **`execute`**: Run `al compile`, git, and other shell commands.
- **`execute: git diff` / `git status`**: See what changed.
- **`agent`**: Delegate to another agent. **the available task-list tool (or update the plan document)**: Track multi-step work.

#### Documentation (MCP)
- **microsoft-docs**: Search Microsoft Learn / BC documentation.
- **context7**: Library and framework documentation.
- **`web` / `web`**: Open web lookups when the MCP docs don't cover it.
...
```
# Build & Validate
execute: al compile                       # Compile + package; read output for errors
execute: al workspace compile             # Multi-project workspace
...
execute: git diff                         # See what changed
...
- **Architecture design needed** → Use the agent tool to delegate to agent `al-architect` with context: "Design solution architecture for this requirement"
- **TDD orchestration needed** → Use the agent tool to delegate to agent `al-conductor` with context: "Orchestrate TDD implementation for this feature"
