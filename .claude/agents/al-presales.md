...
> Canonical source: `agents/al-presales.agent.md` (sha256 `bb2e3fad1905c60115eab83de46ef360b4c423bc8e1b55030b82f4df909af280`).
...
- ✅ Execute `/aldc:al-spec-create` workflow for specifications
...
Create .claude/plans/{req_name}.architecture.md"
```

### Update Global Memory

After completing the presales analysis, **ALWAYS** append a summary to `.claude/plans/memory.md` (append-only, never delete existing content):
...
```markdown
**Action**: Execute /aldc:al-spec-create

Parameters:
- FeatureName: [project-name from intake]
- Scope: [defined scope from Phase 1]

Output: .claude/plans/{req_name}.spec.md
```

### Handoff Contracts

When handing off to other agents, ensure requirement contracts exist in `.claude/plans/`:
- `{req_name}.architecture.md` → Created by @al-architect (COPY from `${CLAUDE_PROJECT_DIR}/claude-plugin/docs/templates/architecture-template.md`)
- `{req_name}.spec.md` → Created by al-spec.create (COPY from `${CLAUDE_PROJECT_DIR}/claude-plugin/docs/templates/spec-template.md`)
...
This agent draws on the following skill from ${CLAUDE_PROJECT_DIR}/.claude/skills/. It is **not** auto-loaded — **load the `SKILL.md` on demand** (read it) when estimating:
