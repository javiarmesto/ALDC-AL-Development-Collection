...
tools: Read, Glob, Grep, LSP, Write, Edit, Bash, Task, WebSearch, WebFetch, mcp__al-symbols-mcp__*, mcp__plugin_aldc_al-symbols-mcp__*, mcp__context7__*, mcp__plugin_aldc_context7__*, mcp__microsoft-docs__*, mcp__plugin_aldc_microsoft-docs__*, mcp__al__al_symbolsearch, mcp__al__al_getdiagnostics, mcp__al__al_getpackagedependencies
model: sonnet
color: blue
---
...
- **Documentation-Driven**: After user approval, COPY `${CLAUDE_PROJECT_DIR}/claude-plugin/docs/templates/architecture-template.md` → `.claude/plans/{req_name}/{req_name}.architecture.md`. **Never edit templates directly.**
...
3. **COPY** `${CLAUDE_PROJECT_DIR}/claude-plugin/docs/templates/architecture-template.md` → `.claude/plans/{req_name}/{req_name}.architecture.md` (kebab-case)
...
Read [the BCQuality design guidance](${CLAUDE_PROJECT_DIR}/claude-plugin/docs/templates/bcquality-design-guidance.md)
...
This agent draws on these skills from `${CLAUDE_PROJECT_DIR}/.claude/skills/`. They are **not** auto-loaded — **load the `SKILL.md` on demand** (read it) when the design enters that domain:
...
The `> **Skills applied**:` line at the top of the architecture document is **mandatory**. Format and placement are defined in `${CLAUDE_PROJECT_DIR}/claude-plugin/docs/templates/architecture-template.md`. List only skills actually loaded; write "None (general architecture patterns only)" if no skill was applied. The Conductor and Review Subagent use this line to verify skill coverage downstream. Next to it, the `> **BCQuality**:` evidence line defined in the design guidance is mandatory whenever the corpus was read, and states `loaded`, never `executed`. Write `> **BCQuality**: not consulted (<reason>)` otherwise.
...
Read [section 14 of the architecture template](${CLAUDE_PROJECT_DIR}/claude-plugin/docs/templates/architecture-template.md)
...
When you write `{req_name}.architecture.md`, **read and fill `${CLAUDE_PROJECT_DIR}/claude-plugin/docs/templates/architecture-template.md`**. The template is immutable and is the single source of truth for: required sections (14 for MEDIUM/HIGH), document header (Date, Complexity, Author, Status), the `> Skills applied:` traceability line, and the Status lifecycle (`Proposed` → `Approved` → `Implemented` → `Superseded`).
