...
const GENERATED_MARKER = '> **Claude Code adapter — generated by';
const CROSS_SURFACE = [
...
for (const [name, marker, what] of CROSS_SURFACE) {
  ...
  const pluginText = read(plugin);
  // Un fichero generado cita su origen canónico y su sha256; su fidelidad ya la
  // comprueba `node scripts/sync-plugin-support.js --check`, que además detecta
  // huérfanos. Repetir aquí el marcador solo añadiría un segundo sitio que mantener.
  if (pluginText.includes(GENERATED_MARKER)) { generatedRoles++; continue; }
...
if (generatedRoles === CROSS_SURFACE.length)
  ok.push('claude-plugin se genera entero: la divergencia entre estirpes la cubre sync-plugin-support --check');
