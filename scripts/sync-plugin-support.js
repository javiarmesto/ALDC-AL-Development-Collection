...
function support(surface, rootDir = root) {
  ...
  if (plansRoot !== CANONICAL_PLANS_ROOT) files.set('aldc.yaml', surfaceAldcYaml(rootDir, surface, plansRoot));
  return files;
}

const CANONICAL_PLANS_ROOT = '.github/plans';

/** The `aldc.yaml` a non-Claude plugin ships so its plans root is declared, not implied. */
function surfaceAldcYaml(rootDir, surface, plansRoot) {
  const sourceRel = 'aldc.yaml';
  const raw = fs.readFileSync(path.join(rootDir, sourceRel));
  const text = raw.toString('utf8')
    .replace(/^toolkitRoot:.*$/m, 'toolkitRoot: "."')
    .replace(/^(plans:\n\s+root:\s*)"[^"]*"/m, `$1"${plansRoot}"`);
  return `# ALDC — ${surface === 'codex' ? 'Codex' : 'Copilot CLI'} toolkit root configuration.\n` +
    `# Generated by scripts/sync-plugin-support.js from the repository aldc.yaml\n` +
    `# (sha256 ${sha256(raw)}). Do not edit; edit the source and re-run the generator.\n` +
    `#\n` +
    `# Requirement artifacts live under ${plansRoot} in a ${surface === 'codex' ? 'Codex' : 'Copilot CLI'} deployment\n` +
    `# (plans.root below). The canonical file keeps ${CANONICAL_PLANS_ROOT}, which is correct\n` +
    `# for the Copilot deployment and the VS Code extension package.\n\n${text}`;
}
...
module.exports = { support, build, sync, AGENTS, ENTRY_SKILLS, WORKFLOWS, workflowSkillName, mappingTable, rewritePaths, plansRootFor, PLUGIN };
