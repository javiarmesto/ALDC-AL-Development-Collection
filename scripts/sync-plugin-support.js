...
function support(surface, rootDir = root) {
  ...
  files.set('surface.json', JSON.stringify({ surface, plansRoot: '.github/plans' }, null, 2) + '\n');
  files.set('project-guidance.md', projectGuidance(surface, '.github/plans'));
  return files;
}
...
function plansRootFor(rootDir) {
  const cfg = yaml.load(fs.readFileSync(path.join(rootDir, 'aldc.yaml'), 'utf8')) || {};
  const canonical = cfg.plans?.root || '.github/plans';
  // Per-surface plans root: Claude Code keeps requirement artifacts beside its
  // own configuration, Copilot and the VSIX keep the canonical .github/plans.
  return canonical.replace(/^\.github\//, '.claude/');
}
