...
const { translateHost, roleTooling } = require('./codex-al-adapter');
...
// reapplied over the role config; inspect the effective runtime policy separately.
...
function expected(root = ROOT) {
  ...
  files.set('skills/aldc/references/al-tooling.md', read('scripts/codex-al-tooling.md'));
  sources.push('scripts/codex-al-adapter.js');
  ...
  for (const p of walk(root,'agents').filter(p => p.endsWith('.agent.md'))) {
    ...
    const body = preface + roleTooling(name) + adapted;
