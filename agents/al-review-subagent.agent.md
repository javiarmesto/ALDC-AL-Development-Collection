...
>
> **Standalone fallback only** (no decision passed — you were invoked outside the Conductor): resolve it yourself — read `aldc.yaml → external.bcquality.enabled`; `false` → skip as above; `auto`/`true` → **probe once** (`read_file <home>/<entryPoint>`, e.g. `../bcquality/skills/entry.md`): a successful read is the mounted signal (proceed), absent → skip as above. A missing knowledge layer **never** fails or blocks the review.
