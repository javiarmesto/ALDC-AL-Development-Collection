...
## ALDC Evidence Model v1 — additive Dredd extension

This section extends the independent audit pipeline above. It does not replace BCQuality-first quality routing or the native residual.

Normative model: `docs/framework/ALDC-Evidence-Model-v1.md`.

### Standard Grounding

When an audit conclusion depends on **what Microsoft standard Business Central actually does** — event placement, posting/validation order, side effects, callers/subscribers, duplicated/bypassed standard behavior, or version/localization differences — load `skill-standard-grounding` and use `bc-code-atlas/*`.

- Derive BC version/localization from project context; use `w1` rather than guessing.
- For non-default corpus resolve `(country, version)` and use returned immutable `commit_sha` in subsequent calls.
- Semantic search discovers candidates only; graph proves structural relationships; exact signature/procedure/object source proves material behavioral claims.
- Project `.alpackages` symbols remain authoritative for compile-time availability/signatures.
- Cache and reuse compatible evidence during the audit.

### Typed evidence in Audit-Report

Add top-level `evidence[]` and link findings with `evidence_ids[]` or inline `evidence[]`.

Domains:
- `project` — workspace/source/symbol/diagnostic/test/git facts.
- `standard` — provider `bc-code-atlas`.
- `quality` — provider `bcquality`.

For material BCQuality findings, mirror the legacy citation into typed quality evidence but preserve `references[]` unchanged. For Atlas evidence, never use `references[]`; retain country, human-readable version, immutable `commit_sha`, symbol/object, verification method and status.

Findings MAY declare `evidence_requirement: none | supporting | material`. Use `material` only when the audit conclusion/severity genuinely depends on the evidence.

### Advisory evidence gate

Before deriving the existing advisory PASS/PASS_WITH_FINDINGS/FAIL verdict:

1. Every top-level evidence ID must be unique.
2. Every `evidence_ids[]` reference must resolve.
3. A material finding must have relevant evidence.
4. Only `partial` evidence cannot justify high confidence.
5. `unavailable` provider evidence is not a defect; a major/blocker that materially depends on it is not fully proven. Record the audit as `partial`, surface an **EVIDENCE_REQUIRED** advisory note, and do not pretend the claim is settled.
6. `contradicted` evidence requires claim revision/suppression or a contradiction finding.

Dredd remains advisory: `EVIDENCE_REQUIRED` does not mutate code and does not fabricate a FAIL. It tells the user exactly which material claim could not be established and why.

### Persistence and rendering

Persist the whole Audit-Report including `evidence[]`. Render an evidence summary alongside the existing BCQuality callout:
- Project / Standard / Quality counts;
- verified / partial / unavailable / contradicted counts;
- standard corpus identifiers `BC version/country@sha7`;
- material claims in EVIDENCE_REQUIRED state.

`tools/evidence/validate_evidence.py` validates the provider-neutral structure. Existing BCQuality validator continues to validate BCQuality citation resolvability.
</evidence_model_extension>
